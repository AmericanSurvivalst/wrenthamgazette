...
// Revalidate every 5 minutes to pick up new content
...
export default async function HomePage() {
  ...
  const townGovArticles = await getArticlesByCategory('town-government', 3);
  const sportsArticles = await getArticlesByCategory('sports', 3);
  const communityArticles = await getArticlesByCategory('community', 3);
  const businessArticles = await getArticlesByCategory('business', 3);
  
  const upcomingEvents = await getUpcomingEvents(4);
  const policeLogs = await getRecentPoliceLogs(4);
